...
3. 段内采用余弦缓入/缓出；每个 chunk 严格输出 32 条 control，下一个 chunk 自动继承上一个 chunk 的末位姿。
4. GUI 通过 SSH 上传首帧和 controls，在 H100 建立 source-only session，运行正式 Pi3X W0 → ReCal3R → WAH + GeoToken 推理。
5. 完成后自动下载合并视频、像素视频、Warp 视频、关联 mask 和指标文件。
