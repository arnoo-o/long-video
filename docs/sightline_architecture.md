...
The Q/K-only binding is applied after native Q/K normalization and RoPE. Values,
FFN, cross-attention, and backbone activations are untouched. A single scalar
`alpha` is zero-initialized and is the only global gate. Short history is bounded
