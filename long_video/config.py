...
class SightlineConfig:
    ...
    geometry_projector_weight_decay:float; geometry_rmsnorm_weight_decay:float; geometry_gate_learning_rate:float; geometry_beta_learning_rate:float
...
def load_sightline_config(path: str|Path) -> SightlineConfig:
    ...
    if not 0.<float(raw['rho_init'])<1.: raise ValueError('rho_init must be strictly inside (0, 1)')
    if raw['scale_augmentation_range'][0]>=raw['scale_augmentation_range'][1] or raw['lora_rank'] not in (8,16) or raw['lora_scope'] not in ('q_k_v_o','disabled') or raw['memory_pool']!=2 or raw['memory_budget']<1 or raw['memory_tau_pos']<=0 or raw['memory_tau_angle']<=0: raise ValueError('invalid memory/LoRA configuration')
    ...
    if min(raw['learning_rate'],raw['lora_learning_rate'],raw['memory_learning_rate'],raw['geometry_gate_learning_rate'],raw['geometry_beta_learning_rate'])<=0: raise ValueError('invalid optimizer learning rate')
    if raw['geometry_projector_weight_decay']<0 or raw['geometry_rmsnorm_weight_decay']<0: raise ValueError('invalid Geometry weight decay')
