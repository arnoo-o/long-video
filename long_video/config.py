...
class SightlineConfig:
    ...
    lora_layers:tuple[int,...]; lora_rank:int; memory_pool:int; memory_budget:int; memory_tau_pos:float; memory_tau_angle:float; lambda_corr:float; lambda_corr_final:float
...
def load_sightline_config(path: str|Path) -> SightlineConfig:
    ...
    if float(raw['alpha_init']) != 1.0: raise ValueError('per-layer Sightline alpha_q/alpha_k must initialize to 1.0')
    if raw['scale_augmentation_range'][0]>=raw['scale_augmentation_range'][1] or raw['lora_rank'] not in (8,16) or raw['memory_pool']!=2 or raw['memory_budget']<1 or raw['memory_tau_pos']<=0 or raw['memory_tau_angle']<=0: raise ValueError('invalid memory/LoRA configuration')
    ...
    if (raw['p1_steps'],raw['p2_steps'],raw['p3_steps'])!=(400,600,1500) or raw['ddp_world_size'] not in range(1,5) or raw['checkpoint_every']!=100 or raw['diagnostics_frequency']!=5: raise ValueError('formal schedule must be P1/P2/P3=400/600/1500, DDP=1..4, metrics=5, checkpoint=100')
