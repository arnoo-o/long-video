...
def token_grid_for_hidden_states(hidden_states: torch.Tensor, patch_size) -> tuple[int, int]:
    patch = tuple(int(value) for value in patch_size)
    height, width = int(hidden_states.shape[-2]), int(hidden_states.shape[-1])
    if height % patch[1] or width % patch[2]:
        raise RuntimeError(f"Helios grid {(height, width)} is not divisible by patch {patch[1:]}")
    return height // patch[1], width // patch[2]
...
class PointWorldGeoTokenProvider:
    ...
    def _build(self, kwargs):
        ...
        patch = kwargs.pop("_geotoken_patch_size", (1, 2, 2))
        stage = stage_for_hidden_states(hidden)
        current_h, current_w = token_grid_for_hidden_states(hidden, patch)
