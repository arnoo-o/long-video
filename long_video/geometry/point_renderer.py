...
def render(node,cameras,near=.05,far=100.,point_radius=1,depth_epsilon=1e-5,device=None,chunk_points=1_000_000):
    ...
    outputs=[]
    for ti in range(t):
        inv=torch.linalg.inv(poses[ti]); zbuf=torch.full((h*w,),float("inf"),device=dev); best=torch.full((h*w,),-1,dtype=torch.long,device=dev)
        ...
        for start in range(0,len(xyz),chunk_points):
            ...
            for dy in range(-point_radius,point_radius+1):
                for dx in range(-point_radius,point_radius+1):
                    ...
                    if len(selected): best[y[selected]*w+x[selected]]=selected+start
