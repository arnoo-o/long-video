...
class SightlineConditioner(nn.Module):
    def __init__(self, inner_dim: int, hidden_dim: int = 128, eps: float = 1e-6,
        ...
        self.gate=nn.Sequential(nn.Linear(3,32),nn.SiLU(),nn.Linear(32,1))
    ...
    def forward(self, rays, *, training=None):
        ...
        g=torch.sigmoid(self.gate(torch.cat((s,s_in,s-s_in),-1)))
        q_in=torch.cat((rays[...,:3],rays[...,3:6],s),-1)
        k_in=torch.cat((rays[...,3:6],rays[...,:3],s),-1)
        dim=self.q_proj[-1].out_features
        q=g*torch.nn.functional.rms_norm(self.q_proj(q_in), (dim,), eps=self.eps)
        k=g*torch.nn.functional.rms_norm(self.k_proj(k_in), (dim,), eps=self.eps)
        return self.alpha*q, self.alpha*k
