...
class SightlineConditioner(nn.Module):
    def __init__(self,inner_dim:int,scale_aug_prob:float=.3,scale_aug_range=(-1.2,1.6),rho_init:float=.2):
        ...
        if not 0.0 < float(rho_init) < 0.4: raise ValueError('rho_init must be strictly inside (0, 0.4)')
        beta_init=torch.logit(torch.tensor(float(rho_init)/0.4))
    ...
    def rho_values(self):
        return 0.4*torch.sigmoid(self.beta_q), 0.4*torch.sigmoid(self.beta_k)
    ...
    def project(self,rays,native=None,*,kind:str,training=None,scale_delta=None,detach_rho:bool=False):
        ...
        output=token_blocked_sightline_relative_project(rays,native,projection,self.gate,norm,beta.detach() if detach_rho else beta,kind=kind,scale_delta=scale_delta,token_tile=self.token_tile,eps=1e-6,rho_max=.4)
