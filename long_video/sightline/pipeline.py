...
class SightlinePipeline:
    ...
    def _capture_clean_memory(self, chunk_index, clean_latent, capture_fn):
        ...
        # Clean capture is intentionally geometry-free.  It archives native
        # hidden states; Sightline K is injected exactly once when read.
    ...
    def generate(self, *, image, prompt, negative_prompt, height, width, num_frames, steps=None, c2w, intrinsics, attention_kwargs=None, boundary_off_from_chunk=None):
        ...
        for chunk in range(chunks):
            ...
            # Chunk zero is a real clean-boundary sample as well: source must
            # enter the native three-stage boundary flow from stage 0 onward.
            boundary=source if chunk==0 else accumulated[:,:,-1:]
            ...
            if constrain_boundary: latent[:,:,0:1]=boundary.to(latent)
