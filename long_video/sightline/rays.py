...
def plucker_rays(c2w: torch.Tensor, intrinsics: torch.Tensor, height: int, width: int,
    ...
    """
    c2w = _as_batched(c2w, 3); K = _as_batched(intrinsics, 3)
    B = c2w.shape[0]; device, dtype = c2w.device, c2w.dtype
    ph, pw = patch_size
    ys = (torch.arange(height, device=device, dtype=dtype) + .5) * ph - .5
    xs = (torch.arange(width, device=device, dtype=dtype) + .5) * pw - .5
    yy, xx = torch.meshgrid(ys, xs, indexing="ij")
    pix = torch.stack((xx, yy, torch.ones_like(xx)), -1).reshape(1,-1,3).expand(B,-1,-1)
    ...
    out = torch.cat((d, m / mn, mn.log()), -1).reshape(B,1,height,width,7)
