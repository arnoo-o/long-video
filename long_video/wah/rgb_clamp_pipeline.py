...
def clamp_enabled(stage_id: int, step_id: int) -> bool:
    return int(stage_id) == 2 and 0 <= int(step_id) < 3
...
class RGBClampWarpAsHistoryPipeline(WarpAsHistoryPipeline):
    ...
    def stage2_sample(self,*args,**kwargs):
        ...
        try:
            ...
            if [x["rgb_clamp"] for x in self._rgb_clamp_diagnostics if x["stage_id"]==2]!=list(STAGE2_CLAMP_STATES): raise RuntimeError("Stage2 RGB clamp schedule must be [1,1,1,0]")
            return sampled
