...
def _frame_cache(xyz, valid, confidence, c2w, K, stride=1):
    ...
    for f in range(frames):
        ...
        out.append({'pixels':pixels,'points':points,'confidence':conf,'tree':tree,'median_depth':median,'depth':depth})
    return out
...
def _point_correspondences(xyz,valid,zbuffer_valid,confidence,query_frame,key_frame,c2w,K,token_height,token_width,distance_fraction,point_stride=1,frames_cache=None):
    ...
    if frames_cache is not None:
        distance,q_to_k=kcache['tree'].query(query,k=1); reverse,_=qcache['tree'].query(key,k=1); keep=(q_to_k>=0)&(reverse[q_to_k] <= distance+1e-7)&(distance<=threshold)
    else:
...
def main():
    p=argparse.ArgumentParser(); p.add_argument('--xyz',type=Path,required=True); p.add_argument('--valid',type=Path,required=True); p.add_argument('--confidence',type=Path,required=True); p.add_argument('--out',type=Path,required=True); p.add_argument('--c2w',type=Path,required=True); p.add_argument('--intrinsics',type=Path,required=True); p.add_argument('--confidence-threshold',type=float,default=0.); p.add_argument('--distance-fraction',type=float,default=.015); p.add_argument('--token-height',type=int,required=True); p.add_argument('--token-width',type=int,required=True); p.add_argument('--min-frame-gap',type=int,default=1); p.add_argument('--screening-stride',type=int,default=32); p.add_argument('--screening-distance-threshold',type=float,default=.05); p.add_argument('--min-overlap-count',type=int,default=1); p.add_argument('--min-overlap-ratio',type=float,default=.01); p.add_argument('--min-count',type=int,default=1); p.add_argument('--min-coverage',type=float,default=0.); p.add_argument('--near-top-ratio',type=float,default=.9); p.add_argument('--point-stride',type=int,default=1); p.add_argument('--stage-a-cache',type=Path); p.add_argument('--trajectory-id',default=''); p.add_argument('--screen-only',action='store_true'); a=p.parse_args()
    ...
    for query in range(frames) if not candidates else []:
        for key in range(0,max(0,query-a.min_frame_gap+1)):
            ...
            if candidate['accepted']:
                accepted+=1
                if not a.screen_only: points.extend(_point_correspondences(xyz,valid,zbuffer_valid,confidence,query,key,c2w,K,a.token_height,a.token_width,a.distance_fraction,a.point_stride,frames_cache))
    ...
    if candidates and not a.screen_only and not points:
        for candidate in candidates:
            if candidate.get('accepted'):
                points.extend(_point_correspondences(xyz,valid,zbuffer_valid,confidence,int(candidate['query_frame']),int(candidate['key_frame']),c2w,K,a.token_height,a.token_width,a.distance_fraction,a.point_stride,frames_cache))
