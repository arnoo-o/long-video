#!/usr/bin/env python3
"""Initialize a causal Pi3 node_000 from one source image."""
...
def main():
    ...
    image = ImageOps.fit(
        image, (args.width, args.height), method=Image.Resampling.LANCZOS,
        centering=(0.5, 0.5),
    )
    ...
    intrinsics = np.repeat(intrinsics[None], 8, axis=0)
    shape = rgb.shape[:3]
    views = ViewSet(
        rgb=rgb,
        ...
        source=np.zeros(shape, np.int8),
        image_confidence=np.ones(shape, np.float32),
        depth_convention=Z_DEPTH,
    )
    backend = Pi3GeometryBackend(
        args.pi3_checkpoint, args.pi3_repo, args.device,
    )
    node = initialize_spatial_node(
        views, backend,
        {
            "node_store": NodeStore(args.output_session),
            "node_id": "node_000",
    ...
    summary = {
        ...
        "prepared_source_image": str(source_path),
        "resolution": [args.height, args.width],
        "fov_degrees": args.fov_degrees,
        "voxel_size": args.voxel_size,
        "point_count": int(len(node.points_xyz)),
