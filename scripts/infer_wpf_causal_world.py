#!/usr/bin/env python3
"""Run causal point world -> renderer -> original WAH + WPF."""
...
def main():
    ...
    node=build_from_views(
        ...
        created_frame=stored_node.created_frame,voxel_size=0.2,status='active',
    )
    node.scale=stored_node.scale; node.model_versions=stored_node.model_versions
    node.quality_metrics.update(stored_node.quality_metrics,source_voxel_size=0.2)
