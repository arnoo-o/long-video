...
def measured_row(capture):
    ...
    if baseline: =0.0
        ...
        =float(torch.stack(gains).mean())
    baseline_final_stage_loss=float(capture['baseline_final_stage_loss'])
    return {"layer":int(capture["layer"]),"sigma":float(capture["sigma"]),"ranking_source":"raw_qk" if baseline else "raw_qk","correspondence_mrr":raw_mrr if baseline else corr_mrr,"top1":raw_top1 if baseline else corr_top1,"top5":raw_top5 if baseline else corr_top5,"raw_qk_mrr":raw_mrr,"raw_qk_top1":raw_top1,"raw_qk_top5":raw_top5,"raw_qk_mrr":None if baseline else corr_mrr,"raw_qk_top1":None if baseline else corr_top1,"raw_qk_top5":None if baseline else corr_top5,"":,
