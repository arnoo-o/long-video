...
def progress_summary(run_dir: Path):
    ...
    if not metrics:
        ...
        return json.loads(status.read_text()) if status.exists() else {"global_step": 0, "total_steps": 1400}
    ...
    return {
        ...
        "stage2_losses": [latest.get("stage2_step2_loss"), latest.get("stage2_step3_loss")],
        "gen_loss": latest.get("gen_loss"), "keep_loss": latest.get("keep_loss"),
        "point_mask_ratio": latest.get("point_mask_ratio"),
