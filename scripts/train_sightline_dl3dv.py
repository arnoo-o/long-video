...
def _generate_detached_chunk(pipe,source,history,prompt_embeds,cfg,chunk):
    ...
    return pipe.stage2_sample(latents=noise,pyramid_num_stages=3,pyramid_num_inference_steps_list=list(cfg.pyramid_steps),
        ...
        attention_kwargs={'current_chunk':chunk},device=source.device,transformer_dtype=getattr(getattr(pipe,'transformer',None),'dtype',source.dtype),progress_bar=Progress())

def _load_correspondence(path):
    payload=json.loads(Path(path).read_text())
    if payload.get('schema_version')!='sightline-correspondence-v5': raise RuntimeError('stale correspondence cache; rebuild with long-memory rows')
