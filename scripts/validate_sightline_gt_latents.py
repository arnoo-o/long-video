...
def main():
    p=argparse.ArgumentParser(); p.add_argument('--overlap',required=True); p.add_argument('--continuous',required=True); p.add_argument('--out',required=True); p.add_argument('--model-provenance',required=True); p.add_argument('--boundary-count',type=int,default=5); p.add_argument('--atol',type=float,default=0.05); a=p.parse_args()
    ...
    result={'passed':bool(float(delta.max())<=a.atol),'model_provenance':a.model_provenance,'overall':overall,'boundary':{'indices':boundary.tolist(),'max_abs':float(boundary_delta.max()),'mean_abs':float(boundary_delta.mean())},'atol':a.atol}
