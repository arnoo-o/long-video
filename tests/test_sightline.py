...
def test_token_blocked_sightline_projection_matches_reference(kind,dtype):
    ...
    norm=torch.nn.RMSNorm(64,eps=1e-4,device=device,dtype=torch.float32)
    ...
    ref_gate=torch.nn.Linear(1,64,device=device,dtype=torch.float32); ref_norm=torch.nn.RMSNorm(64,eps=1e-4,device=device,dtype=torch.float32)
...
def test_token_blocked_sightline_v2_mlp_matches_reference(kind):
    ...
    norm=torch.nn.RMSNorm(32,eps=1e-4).to(device); alpha=torch.nn.Parameter(torch.tensor(.7,device=device)); delta=torch.tensor(.25,device=device)
    ...
    reference=aa*gated*torch.nn.functional.rms_norm(raw,(32,),nw,1e-4)
